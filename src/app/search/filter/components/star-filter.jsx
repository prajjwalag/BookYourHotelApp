          ...
          <FormItem>
            {STAR_FILTERS.map((star) => (
              <FormItem key={star.id} className="flex items-center gap-2">
                <FormControl>
                  <Checkbox className='w-5 h-5' checked={field.value?.includes(star.value)}
                  onCheckedChange={(checked) => {
                    const newValue = checked ? [...field.value, star.value] : field.value?.filter((value) => value !== star.value);
